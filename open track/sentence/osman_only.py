...
def load_all_training_data(barec_path, external_df=None):
    ...
    try:
        ...
        train_df.rename(columns={'Sentence': 'text', 'Readability_Level_19': 'label'}, inplace=True)
...
if train_df is not None:
    ...
    train_dataset = ReadabilityDataset(texts=train_df.text.to_list(), labels=train_df.label.to_list(), tokenizer=tokenizer)
    val_dataset = ReadabilityDataset(texts=val_df.text.to_list(), labels=val_df.label.to_list(), tokenizer=tokenizer)

    model = AutoModelForSequenceClassification.from_pretrained(
        MODEL_NAME, num_labels=NUM_LABELS, ignore_mismatched_sizes=True
    )

    # 🟢 SOLUTION 2: Skipping torch.compile as Triton is not installed.
    # To enable it, install Triton (pip install triton) on a Linux or WSL2 environment.
    print("\n--- ⚠️ Skipping torch.compile as it requires a specific environment setup. Running in standard mode. ---")
    ...
    # Using the standard Trainer since we are not compiling the model.
    trainer = Trainer(
    ...
    try:
        ...
        test_dataset = ReadabilityDataset(
            texts=test_texts, labels=[0] * len(test_texts), tokenizer=tokenizer
        )
